...
pops <- read.csv(paste0(PATH, sep = "/", "multi-year_HYWR_sample_tracksheet.csv"))
pops <- pops[,c(1,2,4)]
names(pops) <- c("indiv", "age", "pop")
pops[199,1] <- "19-074"
pops <- pops %>% 
  mutate(age = case_when(age == 'Buccal' ~ 'Adult', age == 'Tail' ~ 'Larvae', age == 'Toe' ~ 'Adult', TRUE ~ age),
...
#-----------------------------------#
#### combine 2015 + 2019, format ####
#-----------------------------------#
#read in data
aztf2019 <- read.csv(paste0(PATH_19, "/AZTF19_GenotypeData.csv"))
...
aztf_loci <- full_join(aztf2015.v2, aztf2019)
...
microsat <- microsat[,c(18:21,1:17)]

write.csv(microsat, file = paste0(PATH, "/microsat_data/combined_aztf_loci_", Sys.Date(), ".csv"), row.names = FALSE)
